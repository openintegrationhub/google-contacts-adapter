...
/**
 * @desc Delete function which attempts to delete an object based on a supplied recordUid
 *
 * @access  Private
 * @param {String} token - An authorization/access token
 * @param {String} recordUid
 * @return {Object} - Information about success and timestamp of delete operation
 */
async function deleteObject(token, recordUid) {
  if (!recordUid) {
    console.error('Attempted to delete object without id supplied');
    return {
      status: 'failed',
    };
  }

  const uri = `https://people.googleapis.com/v1/${recordUid.includes('people/') ? recordUid : `people/${recordUid}`}:deleteContact`;
  ...
  if (result.statusCode === 200) status = 'confirmed';
  else if (result.statusCode === 403) status = 'denied';
...
module.exports = {
  getObjects, upsertObject, getAccessToken, getSecretsApiEndpoint, deleteObject,
};
